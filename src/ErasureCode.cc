...
int strict_strtol(const char* str, int base, std::string* err) {
    char* endptr;
    errno = 0;
    long val = std::strtol(str, &endptr, base);
    if (errno != 0 || endptr == str || *endptr != '\0') {
        *err = "Invalid integer string: " + std::string(str);
        return 0;
    }
    if (val > INT_MAX || val < INT_MIN) {
        *err = "Integer out of range: " + std::string(str);
        return 0;
    }
...
int ErasureCode::init(ErasureCodeProfile &profile, ostream *ss) {
    _profile = profile;
    return parse(profile, ss);
}

int ErasureCode::create_rule(const string &name, const string &profile, ostream *ss) {
    return 0;
}

unsigned int ErasureCode::get_chunk_count() const {
    return 0;
}

unsigned int ErasureCode::get_data_chunk_count() const { 
    return 0;
}

unsigned int ErasureCode::get_chunk_size(unsigned int object_size) const {
    return object_size;
}

unsigned int ErasureCode::get_alignment() const {
    return SIMD_ALIGN;
}
...
int ErasureCode::decode_concat(const set<int>& want_to_read,
    ...
    if (r == 0) {
        for (unsigned int i = 0; i < get_data_chunk_count(); i++) {
            if (want_to_read.count(chunk_index(i)) &&
                decoded_map.count(chunk_index(i))) {
