			  ...
			  ostream *ss)
{
  ...
  // Instantiate Jerasure plugin directly
  ...
  // Initialize mds and pft erasure codes
  r = mds.erasure_code->init(mds.profile, ss);
  if (r) {
    *ss << "Failed to initialize mds.erasure_code" << std::endl;
    mds.erasure_code.reset();
    pft.erasure_code.reset();
    return r;
  }

  r = pft.erasure_code->init(pft.profile, ss);
  if (r) {
    *ss << "Failed to initialize pft.erasure_code" << std::endl;
    mds.erasure_code.reset();
    pft.erasure_code.reset();
    return r;
  }
			    ...
			    map<int, bufferlist> *repaired, int chunk_size)
{
  ...
  assert(helper_data.size()+aloof_nodes.size()+recovered_data.size() ==
	      (unsigned) q*t);
