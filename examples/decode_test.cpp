// Fixed decode_test.cpp - Compatible with C++14 and correct API
...
int main() {
    ...
    // Test 1: Decode with all chunks (should work perfectly)
    std::cout << "\nTest 1: Decode with all chunks" << std::endl;
    std::set<int> want_decode = {0, 1, 2};
    std::map<int, BufferList> decoded1;
    
    // Use correct decode signature: 4 parameters (last one is typically object_size)
    if (clay.decode(want_decode, chunks, &decoded1, data_size) == 0) {
        std::cout << "Decode successful with all chunks" << std::endl;
        
        // Check if we can reconstruct original
        if (decoded1.count(0) && decoded1.count(1)) {
            // Combine chunks to reconstruct original
    ...
    // Test 2: Decode with missing chunk (fault tolerance)
    std::cout << "\nTest 2: Decode with chunk 2 missing (fault tolerance)" << std::endl;
    ...
    // Test 3: Try to lose 1 data chunk, see if parity helps
    std::cout << "\nTest 3: Decode with chunk 0 missing (using parity)" << std::endl;
    ...
    // Test 4: Simple verification that encoding is not all zeros
    std::cout << "\nTest 4: Verify encoding produces meaningful data" << std::endl;
